...
class Section
{
    ...
    public static function parse($content)
    {
        ...
        preg_match_all('@(?<placeholder_with_tags>' . preg_quote(self::OPEN_TAG) . '\s*(?<placeholder>' . self::PREFIX . '(?<name>[A-Z]+)(\|options\((?<options>.*?)\))?)\s*' . preg_quote(self::CLOSE_TAG) . ')@', $content, $matches, PREG_SET_ORDER);
        if (count($matches)) {
            $instances = [];
